...
if(_dos_open(save_filename, O_RDONLY, &handle) != 0){
  memset(buff, 0, 32);
  if(_dos_open(save_filename, O_WRONLY | O_CREAT, &handle) != 0) return;
...
if(thor_info.last_object > 0 && thor_info.last_object <= NUM_OBJECTS)
  thor_info.last_object_name = object_names[thor_info.last_object - 1];
else
  thor_info.last_object_name = NULL;
#endif
...
int load_music(int num) {
  char* names[] = {
    ...
    "OPENSONG",
    /* GOT1's story screen calls `music_play(7, ...)` (see src/_g1/1_init.c).
