...
void MU_StartMusic(char far* buffer, long length) {
  ...
  /* Stop playback before swapping pointers so MU_Service() can't read
     stale data if got_platform_pump() fires between the free and assign. */
  MU_IsPlaying = 0;
...
void MU_FreeFMMusic(fm_music* m) {
  ...
  /* m->origin points to the caller's song buffer (e.g. the global `song`),
     which is allocated once at init and reused — we must not free it here. */
  free(m);
}
