...
.286
; The original source used TASM-specific directives here. OpenWatcom's WASM
; doesn't understand them, and they are not required for this file.
; MASM51
; QUIRKS
LOCALS
.MODEL MEDIUM,C
...
xdisplay_actors PROC
	        PUBLIC  xdisplay_actors
	        ; The Watcom-compiled C objects in this repo reference a trailing
	        ; underscore variant (e.g. `xdisplay_actors_`). Export an alias at
	        ; the same entry point.
	        PUBLIC  xdisplay_actors_
	        ARG     actor_addr:WORD
	        ARG     dst_page:WORD
	        USES    si,di
...
xerase_actors PROC
        ...
        ; OpenWatcom 16-bit C objects in this project reference a trailing
        ; underscore variant (e.g. `xerase_actors_`). Export an alias label
        ; at the same entry point to satisfy the linker.
        PUBLIC xerase_actors_
        ARG    actor_addr:WORD
        ARG    dst_page:WORD
        USES   si,di
        LOCAL  src_next_off:WORD
...
xput PROC
	        PUBLIC xput
	        ; See note on `xerase_actors_` above.
	        PUBLIC xput_
...
WaitVsyncEnd    macro
        LOCAL   WaitVsync2, WaitNotVsync2
		mov     dx,INPUT_STATUS_0
WaitVsync2:
		in      al,dx
		test    al,08h
...
xshowpage PROC
	        PUBLIC xshowpage
	        ; Export Watcom-expected trailing underscore alias.
	        PUBLIC xshowpage_
	        ARG    Off:WORD
...
wait_vs:
        ...
        ; `main_loop` lives in DGROUP (data). Force DS-based addressing; WASM
        ; can otherwise choose CS-based addressing for this extern, which then
        ; fails to link once the variable is placed in a data segment.
        mov     al,byte ptr ds:main_loop
        cmp     al,0
        jne     pal_ok
        jmp     xshow_done
...
xcopyd2d PROC
	        PUBLIC  xcopyd2d
	        ; OpenWatcom 16-bit C objects in this project reference `xcopyd2d_`.
	        PUBLIC  xcopyd2d_
...
xcopyd2d_:
        ...
        cmp     cx,ax
        jle     xcopy_CopyDone        ;skip if 0 or negative width
        dec     cx
        and     ax,not 011b
        sub     cx,ax
        shr     cx,1
        shr     cx,1    ;# of addresses across rectangle to copy - 1
        jnz     xcopy_MasksSet ;there's more than one address to draw
        and     bh,bl   ;there's only one address, so combine the left
                        ; and right edge clip masks
xcopy_MasksSet:
        mov     ax,SourceEndY
        sub     ax,SourceStartY  ;AX = height of rectangle
        jle     xcopy_CopyDone        ;skip if 0 or negative height
        mov     Height,ax
        mov     ax,DestBitmapWidth
        shr     ax,1            ;convert to width in addresses
        shr     ax,1
...
xcopy_CopyRowsLoop:
        ...
        jz      xcopy_DoRightEdge ;there are only two addresses
...
xfillrectangle PROC
	        ...
	        ; OpenWatcom 16-bit C objects in this project reference `xfillrectangle_`.
	        PUBLIC  xfillrectangle_
	        ARG     StartX:WORD
	        ARG     StartY:WORD
	        ARG     EndX:WORD
	        ARG     EndY:WORD
...
xfr_dne:  jmp     xfr_done

xfr_cnt:  mov     dx,03c4h    ;enable plane 1
        mov	al,2
        out	dx,al
        inc	dx
        mov	al,1
...
xcopys2d PROC
        ...
        jle     s2d_CopyDone        ;skip if 0 or negative width
        mov     RectWidth,cx
        mov     bx,SourceEndY
        sub     bx,SourceStartY  ;BX = height of rectangle
        jle     s2d_CopyDone        ;skip if 0 or negative height
