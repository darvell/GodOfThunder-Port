...
//
// Unified script.c - merged from 1_script.c, 2_script.c, 3_script.c.
// Episode-specific behavior selected at runtime via g_episode / ep pointer.
...
// Superset of internal variables from ep1/ep2 (26 entries).
// Ep3 only uses 24 (@JEWELS..@ITEM) but having the extras is harmless -
// ep3 scripts never reference @THORTILE or @THORPOS.
...
switch(i){
      ...
      case 23:
           ...
           break;
      // @THORTILE and @THORPOS: present in ep1/ep2 scripts only.
      // Ep3 scripts never reference these, so the lookup above
      // will never match them for ep3.
      case 24:
...
// Ep1 uses a larger speech buffer (25000) than ep2/ep3 (20000)
sbuff=farmalloc(g_episode == 1 ? 25000l : 20000l);
...
if(cheat){
  // Ep1 clears a strip and plays a sound; ep2/ep3 just print
  if(g_episode == 1){
    play_sound(DEAD,1);
...
//=========================================================================
// Script EXEC helper functions.
// These are identical across episodes 1/2/3 except where noted.
//=========================================================================
// scr_func1: fall to level 109 (used by all episodes)
void scr_func1(void){
...
//=========================================================================
// scr_func2: random offense/reason strings (used by all episodes)
...
//=========================================================================
// scr_func3: rock-smash / magic hammer tile (ep1 and ep2 only).
// Ep1 uses `while(key_flag[key_magic]) rotate_pal()` for key wait,
// ep2 uses `wait_not_key(key_magic)` -- functionally equivalent.
// We use the ep2 form (wait_not_key) as it is cleaner.
void scr_func3(void){
int p,x,y,o;
...
//=========================================================================
// scr_func4: trigger thunder effect (ep1 and ep2 only)
void scr_func4(void){

thunder_flag=60;
}
//=========================================================================
// scr_func5: troll shrub animation (ep1 only)
void scr_func5(void){
...
//=========================================================================
// Function table for the EXEC script command.
// Ep1 has 5 functions, ep2 has 4, ep3 has 2.
// We include all 5 in the table; if ep3 scripts reference indices 3-5
// they simply won't (those scripts don't exist), but the range check
// in cmd_exec() allows up to 10 safely.
void scr_func1(void);
