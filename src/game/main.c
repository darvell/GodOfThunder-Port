...
//
// Unified main.c - merged from 1_main.c, 2_main.c, 3_main.c.
// Episode-specific differences are resolved at runtime via the episode table.
...
#ifdef __llvm__
/* Build the original DOS entrypoint under a different name so we can provide
   our own native `main()` (raylib). */
#define main got_game_main
#endif

#ifdef __WATCOMC__
/* OpenWatcom inline asm doesn't reliably resolve C enum constants. */
#define atr_Overscan 0x11
#endif

/* Maximum demo key buffer size (ep1=3600, ep2/3=4800). */
#define DEMO_KEY_MAX 4800
...
/* ep2 eyeballs - declared globally, only used when g_episode==2 */
char eyeballs;
...
/* ep2/3 boss intro flags - declared globally */
...
char story_flag;
#ifdef __llvm__
static char _save_fn_buf[64] = "GOTSAVE1.SAV";
char *save_filename = _save_fn_buf;
#else
char *save_filename="XXXXXXXX.XXX";
#endif
char far *scr;
char demo_key[DEMO_KEY_MAX];
int  demo_cnt;
char demo,record;
char demo_enable;
int  rnd_index;
#ifdef __llvm__
uint16_t rnd_array[100];
#else
int  rnd_array[100];
#endif
char rdemo;
...
char main_loop;
int got_wants_quit;  /* Set when user chooses "Quit to DOS" from menu */
char end_tile;

/* ep2/3 endgame globals - declared globally */
int  endgame;
int game_is_over;
...
/* ep2/3 boss intro init */
boss_intro1=0;
boss_intro2=0;
/* ep2/3: set start armor from episode table (ep1 leaves it 0) */
thor_info.armor=ep->start_armor;
startup=1;
hourglass_flag=0;
thunder_flag=0;
lightning_used=0;
...
#ifdef __llvm__
/* Set save filename from episode table */
strncpy(_save_fn_buf, ep->save_file, sizeof(_save_fn_buf) - 1);
_save_fn_buf[sizeof(_save_fn_buf) - 1] = '\0';
#endif
...
if(demo || record){
  if(record) demo=0;
  /* ep1 also sets area=1, setup.area=1 here, but unified already has area from ep */
  cash1_inform=1;
  cash2_inform=1;
  door_inform=1;
  magic_inform=1;
...
#else
memcpy(&scrn, sd_data + (new_level * 512), sizeof(LEVEL));
#endif
...
if(auto_load){
  ...
  auto_load=0;
  /* Check if we're on any boss level and need to resume the boss fight.
   * Ep1 has 3 boss levels; ep2/3 have 1 each.  Iterate the episode table. */
  {
    int bi;
    for(bi=0; bi<ep->num_boss_levels; bi++){
      if(current_area==ep->boss_levels[bi] && !setup.game_over){
        auto_load=1;
        fade_in();
        ep->boss_level_fn[bi]();
        break;
...
if(cheat){
  ...
  if(cheat){
    ...
    if(key_flag[_Z]){
      add_magic(150);
      /* ep1: inventory|=128+256 (shield+thunder), ep2/3: 16+32.
       * Unified: use episode magic constants. */
      thor_info.inventory |= SHIELD_MAGIC | THUNDER_MAGIC;
    }
  ...
  if(opt){
    ...
    if(opt==1) save_game();
      ...
      if(restart_episode()){
        setup_player();
        load_sd_data();
        hourglass_flag=0; thunder_flag=0; lightning_used=0;
        tornado_used=0; shield_on=0; bomb_flag=0; boss_active=0;
        for(i=1;i<MAX_ACTORS;i++) actor[i].used=0;
    ...
    else if(opt==6) debug_menu();
    else if(opt==7){
      extern void got_settings_menu(void);
      got_settings_menu();
      d_restore();
    }
    else if(opt==8){
      xdos=ask_exit();
      if(xdos==3) { got_wants_quit=1; break; }  /* Quit to Desktop */
      if(xdos==2) { break; }                    /* Quit to Opening Screen → launcher */
    }
#endif
    opt=0;
  }
  ...
  for(loop=0;loop<vl;loop++){
    for(i=0;i<ma;i++){
       if(actor[i].used){
         /* Hourglass freeze: ep1 has HOURGLASS_MAGIC!=0, so this check
          * can freeze actors.  For ep2/3 HOURGLASS_MAGIC==0, so the
          * bitwise AND is always 0 and the continue is never reached. */
         if(hourglass_flag)
  ...
  /* ep2: eyeballs direction override */
  if(g_episode==2){
    if(eyeballs){
  ...
  /* ep3: endgame movement */
  if(ep->endgame_movement_fn && endgame) ep->endgame_movement_fn();
  if(end_tile) break;
  brk_loop:
  if(exit_flag==2){
    thor_dies();
  ...
  if(current_level!=new_level){
      ...
      hammer->used=0;
      show_level(new_level);
      /* ep2/3 clear thunder_flag on level change; ep1 does not */
      if(g_episode!=1) thunder_flag=0;
      tornado_used=0;
      if(cheat && !demo){
  ...
  use_item();
#ifdef __llvm__
  { extern int got_platform_get_item_cycle(void);
    int cdir = got_platform_get_item_cycle();
    if(cdir && thor_info.inventory && !thunder_flag && !hourglass_flag
       && !lightning_used && !tornado_used && !shield_on){
      int p = thor_info.item - 1;
      int tries = 9;
      do {
        p += cdir;
        if(p > 8) p = 0;
        if(p < 0) p = 8;
      } while(!(thor_info.inventory & (1 << p)) && --tries > 0);
      if(thor_info.inventory & (1 << p)){
        thor_info.item = p + 1;
        display_item();
        play_sound(WOOP, 1);
      }
  ...
  if(boss_dead){
    ...
    if(loop==7){
      ...
      exit_flag=0;
      /* Iterate boss levels and call the appropriate closing sequence.
       * Ep1 has boss_active==1 check to distinguish which boss;
       * Unified: iterate boss_levels, call closing_fn for the matching level. */
      if(boss_active){
        int bi;
        for(bi=0; bi<ep->num_boss_levels; bi++){
          if(current_area==ep->boss_levels[bi] && ep->closing_fn[bi]){
            ep->closing_fn[bi]();
            break;
          }
        }
...
exit_code(0);
	...
	if((!cheat) || demo){
#if defined(__WATCOMC__) && !defined(__llvm__)
	  {
	    union REGS regs;
	    regs.x.ax = 0x0100;
	    regs.x.cx = 0x2000;
	...
	run_gotm();
#ifdef __llvm__
	return;   /* Return to launcher menu instead of terminating. */
#else
	exit(0);
#endif
	}
...
void run_gotm(void){
#ifdef __llvm__
/* Native build: GOT.EXE (menu/highscore) is not part of this repo/port. */
return;
#endif
int num;
char s[36];
...
play_sound(DEAD,1);
thor_spins(0);
/* ep2: skip tombstone when in eyeballs form */
if(!(g_episode==2 && eyeballs))
  xfput(thor->x,thor->y,display_page,objects[ep->death_obj_index]);
thor->used=1;
timer_cnt=0;
...
thor->next=0;
for(i=0;i<60;i++){
   /* ep2: don't spin thor's direction when in eyeballs form */
   if(!(g_episode==2 && eyeballs)){
     thor->dir=da[d];
     thor->last_dir=da[d];
   }
...
/* ep2: check for eyeballs level on load */
if(g_episode==2 && current_level==105){
  thor_info.armor=2;
  load_new_thor();
  eyeballs=1;
  setup.f25=0;
