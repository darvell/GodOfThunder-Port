...
/* DEMO_LEN is now a runtime value (ep->demo_len), but the demo_key array
   needs a fixed compile-time size.  Use the maximum across all episodes. */
...
extern char demo_key[DEMO_KEY_MAX];
...
//commandeer the KB int (DOS only). The native build polls raylib events instead.
#ifndef __llvm__
...
#ifdef __llvm__
/* Native build: enable full audio — OPL2 music + VOC digital sound effects.
   PC speaker SFX disabled since digital VOC is higher quality. */
setup.music = 1;
setup.dig_sound = 1;
setup.pc_sound = 0;
music_flag = 1;
...
#ifdef __llvm__
{ extern void got_config_set_defaults(void);
  extern int got_config_load(const char *);
  extern void got_config_apply(void);
  got_config_set_defaults();
  got_config_load("GOT.CFG");
  got_config_apply();
}
#endif
...
song=farmalloc(ep->song_buf_size);
if(!song) return 5;
...
xprintx(7*8,15*8,(char *)ep->title,PAGE1,32);

fade_in();
timer_cnt=0;

#ifdef __llvm__
/* Native build: enable split-mode mapping before any status-panel rendering so
   draws to PAGES go to the stat surface (the actual split presentation happens
   later). */
split_screen();
#endif
...
unsplit_screen();
{
  unsigned int saved_dp = display_page;
  display_page = PAGE1;
  while(timer_cnt<240) rotate_pal();
  display_page = saved_dp;
}
#else
while(timer_cnt<240);
#endif
...
#ifdef __llvm__
extern void got_platform_video_shutdown(void);
got_platform_video_shutdown();
#endif
...
#ifdef __llvm__
/* Native build: refresh key_flag[] from the platform before checking it.
   The DOS build relies on the keyboard ISR to keep key_flag current, but the
   native build can enter this function after code has manually cleared key_flag
   entries (e.g. ESC -> option_menu). */
{ extern void got_platform_pump(void); got_platform_pump(); }
#endif
...
#ifdef __llvm__
/* Native build: poll platform input so key_flag[] stays current.
   Without this, callers that spin on get_response() (e.g. wait-for-key-release
   loops) would hang because the DOS keyboard interrupt is absent. */
{ extern void got_platform_pump(void); got_platform_pump(); }
#endif
if(joystick){
  joy_key();
...
#ifdef __llvm__
/* See wait_not_response() note: ensure key_flag[] reflects real key state. */
{ extern void got_platform_pump(void); got_platform_pump(); }
#endif
...
while(1){
     story_animate();
#ifdef __llvm__
     /* Native builds don't have the DOS keyboard ISR updating key_flag[].
        Keep presenting the current page so raylib can poll input events and
        VSYNC can throttle this loop. */
...
for(i=0;i<256;i++){
   ...
   b=pbuff[(i*3)+2];
#ifdef __llvm__
   xsetpal((unsigned char)n, (unsigned char)r, (unsigned char)g, (unsigned char)b);
#else
   asm mov  al,n
...
xshowpage(38400u);
if(res_read(ep->story_res,(char far*)tmp_buff)<0) return;
...
/* Episode 1 places two pictures; episodes 2 and 3 place only one. */
if(g_episode==1){
