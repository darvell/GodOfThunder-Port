...
/* object_names[] is provided per-episode via ep->object_names */
...
for(y=0;y<12;y++){
   for(x=0;x<20;x++){
      if(g_episode == 2 || scrn.icon[y][x]!=0){
...
/* ep2-only: eyeballs/armor/slip setup */
if(g_episode == 2){
  if(new_level==60){
    thor_info.armor=10;
  }
...
#else
memcpy(sd_data + (current_level * 512) + 322, &scrn.static_obj, 130);
memcpy(sd_data + (current_level * 512), &scrn, sizeof(LEVEL));
...
f=1;
/* Check boss levels via episode table */
for(i=0;i<ep->num_boss_levels;i++){
  if(new_level==ep->boss_levels[i]){
    if(!setup.boss_dead[g_episode-1]){
      if(!auto_load) ep->boss_level_fn[i]();
      f=0;
    }
    boss_active=1;
    break;
...
/* ep3: ending screen at level 106 */
if(g_episode == 3 && current_level==106){
  if(ep->ending_screen_fn) ep->ending_screen_fn();
  f=0;
}
...
#ifdef __llvm__
     /* On DOS, VGA continuously scans memory so writes are immediately
        visible.  On native we must explicitly present.  Every 7 tiles
        at 70 fps = 480 ms total, matching the original DOS timing. */
     if(cnt%7==0) xshowpage(display_page);
#endif
     delay(2);
}
#ifdef __llvm__
...
//===========================================================================
/* d_restore() is in grp.c for the unified build */
//===========================================================================
...
/* ep2/3 guard: switch_icons is a no-op during thunder */
if(g_episode != 1 && thunder_flag) return 0;
play_sound(WOOP,0);
for(y=0;y<12;y++){
   for(x=0;x<20;x++){
     ix=x*16;
...
/* ep2/3 guard: rotate_arrows is a no-op during thunder */
if(g_episode != 1 && thunder_flag) return 0;
play_sound(WOOP,0);
for(y=0;y<12;y++){
   for(x=0;x<20;x++){
