...
if(g_episode == 1){
  /* Episode 1: simple damage scaling by skill */
  if(!setup.skill) damage*=2;
  else if(setup.skill==2) damage/=2;
} else {
  /* Episodes 2 & 3: armor bonus, but not during magic attacks */
  if(!thunder_flag && !lightning_used && !tornado_used){
    if(!setup.skill) damage*=2;
    else if(setup.skill==2) damage/=2;
    if(g_episode == 2) d=damage/4;  //for armor (ep2)
    else d=damage/3;                //for armor (ep3)
    damage+=d;
  }
}
...
#ifdef __llvm__
{ extern char debug_god_mode; if(debug_god_mode) return; }
#endif

if(g_episode >= 2){
  /* Episodes 2 & 3: talking NPCs (func_num 11) */
  if(actr->func_num==11){
...
if(g_episode == 1){
  /* Episode 1: simple damage scaling */
  if(damage!=255){
    if(!setup.skill) damage/=2;
    else if(setup.skill==2) damage*=2;
  }
} else {
  /* Episodes 2 & 3: armor damage reduction */
  if(damage<150){
    if(!setup.skill) damage/=2;
    else if(setup.skill==2) damage*=2;
    if(g_episode == 2) d=damage/4;
    else d=damage/3;
    damage-=d;
  }
