...
void boss_set_ep2(int d,int x,int y);
int bossa_movement_ep2(ACTOR *actr);
int bossb_movement_ep2(ACTOR *actr);
//===========================================================================
...
#ifdef __llvm__
#define exp  exp_tbl
#define expf expf_tbl
#endif

static char exp[]={ 61, 62, 65, 66, 69, 70, 73, 74, 77, 78,
...
char num_spikes;
//===========================================================================
int boss_movement_ep2(ACTOR *actr){    //boss - skull
int d,f,x;
...
if(boss_dead) return boss_die_ep2();
if(actr->i1){
  if(actr->i1==1) return bossa_movement_ep2(actr);
  else return bossb_movement_ep2(actr);
}
d=actr->last_dir;
...
//===========================================================================
int bossa_movement_ep2(ACTOR *actr){    //boss - skull (explode)
int an,x,y,r;
...
//===========================================================================
int bossb_movement_ep2(ACTOR *actr){    //boss - skull - shake
int rep,an,hx,hy,d;
...
next_frame(actr);
boss_set_ep2(actr->dir,actr->x,actr->y);
return 0;
}
