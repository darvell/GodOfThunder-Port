...
//
// Unified movpat.c - merged from 1_movpat.c / 2_movpat.c / 3_movpat.c
...
extern char area;
extern int endgame;         // ep2/ep3
extern char eyeballs;       // ep2

// Slip/slide physics (ep2/ep3 only; ep1 never sets slip_flag)
char slip_flag=0;
char slip_cnt=0;
char slipping=0;
...
// ep2: slip_flag reset before tile check
if(g_episode==2) slip_flag=0;

#ifdef __llvm__
{ extern char debug_noclip_mode;
if((cheat+key_flag[_ONE])<2 && !debug_noclip_mode){
#else
...
// ep2: slip/slide logic after tile check
if(g_episode==2){
  if(!slip_flag){
    slipping=0;
    slip_cnt=0;
...
for(i=3;i<MAX_ACTORS;i++){       //max_actors
   ...
   if(!act->used) continue;
   if(g_episode!=1){
     // ep2/ep3: skip boss-flagged actors when boss is active
   ...
   if(overlap(x1,y1,x2,y2,x3,y3,x4,y4)){
     if(g_episode==1){
       // ep1: dispatch boss hit by boss_active type
       if(boss_active){
         switch(boss_active){
           case 1:
       ...
       // ep2: single boss_hit call
       if(boss_active){
         check_boss_hit_ep2(act,x1,y1,x2,y2,i);
         break;
       }
       else{
         if(act->solid==2 && (actr->move==16 || actr->move==17)) return 0;
       ...
       // ep3: boss code was commented out in original; just damage
       if(act->solid==2 && (actr->move==16 || actr->move==17)) return 0;
       actor_damaged(act,actr->strength);
     }
     f++;
...
// ep2: slipping physics
if(g_episode==2 && slipping){
...
// ep2: eyeballs keep animating even when no key pressed
...
//===========================================================================
int special_movement_two(ACTOR *actr){   //angel
int x1,y1,x2,y2,i;
int x3,y3,x4,y4;
ACTOR *act;
...
if(g_episode==1){
  // ep1 uses temp6 for talk cooldown
  if(actr->temp6){
    actr->temp6--;
    return 0;
  }
...
//===========================================================================
int special_movement_eleven(ACTOR *actr){  //red guard (ep2/ep3 only)
int t;
...
if(actr->num_shots>0){
  ...
  actr->dead=2;
  actr->used=0;
  // ep1 has no endgame check; ep2/ep3 check !endgame
  if(g_episode==1){
...
//===========================================================================
int movement_nineteen(ACTOR *actr){    //same as seven

return movement_seven(actr);
}
//===========================================================================
int movement_twenty(ACTOR *actr){
// ep1: boss - snake movement
// ep2/ep3: just frame cycle
if(g_episode==1) return boss1_movement(actr);
...
//===========================================================================
int movement_twentysix(ACTOR *actr){
// ep1: just frame cycle
// ep2: boss (loki) movement
// ep3: boss movement
if(g_episode==1) return movement_one(actr);
if(g_episode==2) return boss_movement_ep2(actr);
return boss_movement_ep3(actr);
}
//===========================================================================
int movement_twentyseven(ACTOR *actr){
// ep1: just frame cycle
// ep2: boss movement
// ep3: just frame cycle
if(g_episode==2) return boss_movement_ep2(actr);
return movement_one(actr);
}
//===========================================================================
