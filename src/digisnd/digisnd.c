...
#ifdef __WATCOMC__
/*
  OpenWatcom accepts `__asm <instr>` statements (which we get via `owpre.h`),
  but the `{ ... }` block form is pickier across targets/dialects. Use the
  statement form to avoid the compiler accidentally staying in "asm mode".
*/
...
{
#ifdef __WATCOMC__
  // OpenWatcom provides a C runtime wrapper.
  return (InterruptHandler)_dos_getvect(num);
#else
